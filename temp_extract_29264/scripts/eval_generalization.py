...
Runs the same checkpoint against several env perturbations:
  ...
  - dense_clutter      : requests rho=1.5; CorridorBuilder clamps density to [0,1],
                         so this often repeats training-case saturation—check CSV vs id row.
  - turns              : sets enable_turns=True via curriculum extras; geometry may still
                         be straight-corridor unless the builder implements bends.
  - narrow_corridor    : 1.6 m width plus lower rho/wind vs training (composite stress test,
                         not an isolated width-only sweep).
