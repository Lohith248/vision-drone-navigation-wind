...
## Implementation pivot (final deliverable)

The narrative below reflects the **original mid-semester plan** (Flightmare + ROS + Stable Baselines3 and a transformer-style critic split). The **completed project pivoted** to a **custom PyBullet / Gymnasium** corridor environment with **in-repo PPO training**, a **shared-trunk multimodal actor–critic** (ViT encoder plus state fusion), and **state-only DDPG/SAC baselines** in the same stack. See `README.md` and `report_artifacts/report/report.tex` for what was actually built and evaluated.
