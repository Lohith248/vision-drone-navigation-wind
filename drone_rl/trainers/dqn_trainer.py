"""
DQN Trainer: off-policy with PER, epsilon-greedy, discrete actions.
"""
...
from drone_rl.algorithms.dqn import DQNAlgorithm
from drone_rl.replay_buffers.prioritized_replay import PrioritizedReplayBuffer
from drone_rl.utils.normalization import ObservationNormalizer
from drone_rl.utils.schedule import LinearSchedule


class DQNTrainer(BaseTrainer):
    """DQN training loop with PER, Double DQN, and Dueling architecture."""

    def __init__(self, config: dict):
        config.setdefault("algorithm", "dqn")
    ...
    def _setup(self) -> None:
        ...
        action_mode = dqn_cfg.get("action_mode", "semantic")

        def make_env():
            base = DroneCorridorEnv(
            ...
            return DiscreteNavWrapper(base, action_mode=action_mode,
                                      n_bins=dqn_cfg.get("n_bins", 3))
        ...
        self._n_actions = self.eval_env.action_space.n

        # Network
        hidden_dims = tuple(dqn_cfg.get("hidden_dims", [256, 256]))
        q_network = DuelingDQN(
            obs_dim=self._obs_dim, n_actions=self._n_actions,
            hidden_dims=hidden_dims,
        )
        print(f"  DQN params: {sum(p.numel() for p in q_network.parameters()):,}")
        print(f"  Actions: {self._n_actions} ({action_mode})")

        # Algorithm
        self.algorithm = DQNAlgorithm(
            q_network=q_network, n_actions=self._n_actions,
            lr=dqn_cfg.get("lr", 1e-4),
            gamma=dqn_cfg.get("gamma", 0.99),
            target_update_freq=dqn_cfg.get("target_update_freq", 500),
            max_grad_norm=dqn_cfg.get("max_grad_norm", 10.0),
        ...
        self.replay_buffer = PrioritizedReplayBuffer(
            capacity=dqn_cfg.get("buffer_size", 50000),
            obs_dim=self._obs_dim, action_dim=1,
            alpha=dqn_cfg.get("per_alpha", 0.6),
            beta_start=dqn_cfg.get("per_beta_start", 0.4),
            beta_end=dqn_cfg.get("per_beta_end", 1.0),
            action_dtype="int64",
        )

        # Obs normalizer
        if dqn_cfg.get("normalize_obs", True):
            self.obs_normalizer = ObservationNormalizer(self._obs_dim)

        # Epsilon schedule
        self.epsilon_schedule = LinearSchedule(
            start=dqn_cfg.get("eps_start", 1.0),
            end=dqn_cfg.get("eps_end", 0.05),
        )

        self._batch_size = dqn_cfg.get("batch_size", 64)
        self._learning_starts = dqn_cfg.get("learning_starts", 1000)
        self._train_freq = dqn_cfg.get("train_freq", 4)
        self._steps_per_iteration = dqn_cfg.get("steps_per_iteration", 1000)
    ...
    def _train_step(self) -> Dict[str, float]:
        ...
        for step in range(self._steps_per_iteration):
            ...
            epsilon = self.epsilon_schedule(progress)
            action = self.algorithm.select_action(obs_norm, epsilon)
            ...
            self.replay_buffer.push(obs, np.array([action]),
                                     float(reward[0]), next_obs_flat, bool(done))
            ...
            if (self.total_timesteps >= self._learning_starts and
                ...
                batch, tree_idx, weights = self.replay_buffer.sample(
                    self._batch_size, self.device, progress
                )
                metrics, td_errors = self.algorithm.update(batch, weights)
        ...
        for _ in range(n_episodes):
            ...
            while not done:
                if self.obs_normalizer:
                    ...
                    obs_norm = obs_flat
                action = self.algorithm.select_action(obs_norm, epsilon=0.0)
